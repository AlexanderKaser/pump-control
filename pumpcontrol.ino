class non_blocking_timed_pin
{
	public:
      non_blocking_timed_pin()
    	{}
        non_blocking_timed_pin(int pin, unsigned long time_span, string name): pin_(pin),time_span_(time_span), name_(name)
        {
        }
    	void digital_write_timed(unsigned int state_start, unsigned int state_end)
        {
        	if(running_)
          {
...
bool start_ventil_1_=true,start_ventil_2=false,start_ventil_3=false;
...
void loop() {
  ...
  if (distance_fish < 12) { //die 12 unbedingt überprüfen sonst zieht es aus Versehen die ganze Zeit Strom
    if(start1)
    {
    	start2 = ventil1.digital_write_timed(HIGH,LOW);
      start1 = !start2;
    }else if(start2)
    {
    	start3 = ventil2.digital_write_timed(HIGH,LOW);
      start2 = !start3;
    }else if(start3)
    {
    	start1 = ventil3.digital_write_timed(HIGH,LOW);
      start3 = !start1;
    }
